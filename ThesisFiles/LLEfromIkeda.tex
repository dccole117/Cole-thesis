...
\end{align}
Using a normalized distance $s=z\cdot \pi/\mathcal{F}L$ we define the operator $G_1$ that advances the field $\psi(s,\theta)$ a distance $\Delta s=\pi/\mathcal{F}$ as $\psi(s+\pi/\mathcal{F},\theta)=G_1 \psi(s,\theta)$ according to a nonlinear Schrodinger equation of the form:
\begin{equation}
